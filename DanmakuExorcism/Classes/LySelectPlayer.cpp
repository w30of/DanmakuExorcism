...
void LySelectPlayer::playAnime(const char * aniName, cocos2d::SEL_CallFunc callfunc /*= nullptr*/)
{
    if (m_AnimationManager == nullptr) {
        m_AnimationManager = m_pAnimationManager;
    }
    
    m_pAnimationManager->runAnimationsForSequenceNamed(aniName);
    if (callfunc) {
        m_pAnimationManager->setAnimationCompletedCallback(this, callfunc);
    }
}
