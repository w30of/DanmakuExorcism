...
// public functions...
void GameLogic::resetPlay()
{
    gamePlayTime = 0;
    isHoldPlayTime = false;
}

void GameLogic::pausePlayTime()
{
    isHoldPlayTime = true;
}

void GameLogic::resumePlayTime()
{
    isHoldPlayTime = false;
}

void GameLogic::createEnemy(float timeDelta)
{
    realPlayTime += timeDelta;
    if (isHoldPlayTime) return;
    gamePlayTime += timeDelta;
    // No enemy to create should pause.
    DataAdapter* data = DataAdapter::getInstance();
    
    if (!data->logoHasShown && gamePlayTime >= data->logoShowTime) {
        if (gLayer) {
