...
void Enemy::die()
{
    ...
    if (m_enemyInfo.isHoldTime) {
        GameLogic::getInstance()->resumePlayTime();
    }
...
//        // Prepare...
    ...
    else if (m_enemyInfo.EnemyID == ENEMY_1_3)
    {
        ...
        this->runAction(Sequence::create(DelayTime::create(3),
                                         Show::create(),
                                         CallFunc::create([=](){ this->setInvisible(false); }),
                                         NULL));
    }
    else
    {
    ...
    if (this->m_enemyInfo.isHoldTime) {
        GameLogic::getInstance()->pausePlayTime();
    }
}
